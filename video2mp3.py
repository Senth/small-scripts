...
def convert_file(filepath: Path):
    ...
    command = ['ffmpeg', '-i', filepath, '-metadata', metadata, '-threads', '4', '-acodec',
               'libmp3lame', '-ac', '2', '-ab', '256k', '-vn', '-y', outfile]

    log_message(
        'Converting ||| {} -----> {}'.format(filepath.as_posix(), outfile))
...
def convert_file_or_dir(file):
    ...
    if filepath.is_dir():
        ...
        if is_video_file(filepath) and (not args.include or file_matches_include(filepath)):
            convert_file(filepath)
